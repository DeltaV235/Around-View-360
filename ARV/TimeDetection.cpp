...
void TimeDetection::getAvgTime()				//获取平均时间 或每次操作所需要的时间
{
...
void TimeDetection::getCurTime()				//获取当前一次操作所使用的时间
{
...
void TimeDetection::setStartPos()			//获取一次操作开始的时间
{
	start = getTickCount();
}
void TimeDetection::setEndPos()					//获取一次操作中结束的时间，并计算该次操作所使用的时间，写入到data[]中
{
	end = getTickCount();
